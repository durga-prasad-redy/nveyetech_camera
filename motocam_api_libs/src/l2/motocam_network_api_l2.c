...
uint8_t set_onvif_interface_l2(const uint8_t onvif_interface_len,
                               const uint8_t *onvif_interface) {
  ...
  if (current_onvif_interface_state == *onvif_interface) {
    LOG_ERROR("current onvif interfact state matches with onvif interface to "
              "be changed ");
    return -2;
  }

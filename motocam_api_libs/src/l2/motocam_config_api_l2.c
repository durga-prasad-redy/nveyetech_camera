...
int8_t get_config_streaming_config_l2(uint8_t **config, uint8_t *length) {
  ...
  char stream_ini_path[256];
  snprintf(stream_ini_path, sizeof(stream_ini_path), "%s/stream.ini",
           CONFIG_PATH);

  // Get stream_count from [streamer] section
  int stream_count =
      get_ini_value(stream_ini_path, "streamer", "stream_count", 2);
  ...
  *config = (uint8_t *)malloc(*length);
  ...
  for (int i = 0; i < stream_count; i++) {
    ...
    // Get bitrate (in bps from stream.ini, convert to Mbps to match original
    // format)
    int bitrate_bps =
        get_ini_value(stream_ini_path, section, "bitrate", 2000000);
    ...
    if (bitrate_mbps < 0) {
      ...
      printf("Warning: Bitrate %d bps (%d Mbps) for stream%d exceeds uint8_t "
             "range, capping at 255\n",

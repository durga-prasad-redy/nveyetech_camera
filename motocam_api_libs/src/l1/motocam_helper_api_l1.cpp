...
int8_t validate_req_bytes_crc(const uint8_t *req_bytes, const uint8_t req_bytes_size)
{
    ...
    auto lower_8_bits = (uint8_t)sum_req_bytes;
    if (lower_8_bits == 0)
        return 0;
                   ...
                   uint8_t **res_data_bytes, uint8_t *res_data_bytes_size)
{
    ...
    {
    case STREAMING:
        return get_streaming_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    case NETWORK:
        return get_network_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    case CONFIG:
        return get_config_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    case IMAGE:
        return get_image_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    case AUDIO:
        return get_audio_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    case SYSTEM:
        return get_system_command(sub_command, data_length, data, res_data_bytes, res_data_bytes_size);
    default:
        printf("invalid command\n");
        return -3;
    }
